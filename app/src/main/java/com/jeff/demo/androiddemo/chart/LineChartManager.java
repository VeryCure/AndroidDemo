...
public class LineChartManager
{
    ...
    private void initLineChart() {
        ...
        xAxis.setGridColor(Color.parseColor("#bbc0c4"));
        xAxis.setAxisLineColor(Color.parseColor("#00bbc0c4"));
        xAxis.setAvoidFirstLastClipping(false);
        xAxis.setGridDashedLine(new DashPathEffect(new float[]{10f,9f},0));
        //保证Y轴从0开始，不然会上移一点
    ...
    private void initLineDataSet(LineDataSet lineDataSet, int color, boolean mode) {
        ...
        lineDataSet.setCircleColor(Color.parseColor("#ff7c00"));
     ...
     * @param color
     */
    public void showLineChart(Context context,List<Float> xAxisValues,
                              List<Float>
            yAxisValues, String label, int color) {
        initLineChart();
        ArrayList<Entry> entries = new ArrayList<>();
        for (int i = 0; i < xAxisValues.size(); i++) {
            entries.add(new Entry(xAxisValues.get(i), yAxisValues.get(i),
                    context.getResources().getDrawable(R.drawable.ic_line_chart_select_dot)));
        }
        // 每一个LineDataSet代表一条线
